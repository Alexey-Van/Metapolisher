...
source /mnt/data/vanichkinao/MetaPolisher/merqury/util/util.sh
export MERQURY=/mnt/data/vanichkinao/MetaPolisher/merqury
...
name=$out.spectra-cn
log=logs/$name.log
/mnt/data/vanichkinao/MetaPolisher/merqury/eval/spectra-cn.sh $compress $readdb $asm1 $asm2 $out &> $log
...
name=$out.blob
log=logs/$name.log
/mnt/data/vanichkinao/MetaPolisher/merqury/trio/hap_blob.sh $hap1 $hap2 $asm1 $asm2 $out &> $log

echo "
Get haplotype specfic spectra-cn plots"
name=$out.spectra-hap
log=logs/$name.log
/mnt/data/vanichkinao/MetaPolisher/merqury/trio/spectra-hap.sh $readdb $hap1 $hap2 $asm1 $asm2 $out &> $log
...
/mnt/data/vanichkinao/MetaPolisher/merqury/trio/phase_block.sh $asm1 $hap1 $hap2 $out.$asm1_name &> $log
echo
...
if [ -z $asm2 ] ; then
	...
	/mnt/data/vanichkinao/MetaPolisher/merqury/trio/block_n_stats.sh $asm1 $out.$asm1_name.*.phased_block.bed $out
...
/mnt/data/vanichkinao/MetaPolisher/merqury/trio/phase_block.sh $asm2 $hap1 $hap2 $out.$asm2_name &> $log
...
/mnt/data/vanichkinao/MetaPolisher/merqury/trio/block_n_stats.sh $asm1 $out.$asm1_name.*.phased_block.bed $asm2 $out.$asm2_name.*.phased_block.bed $out
