...
def main(args):
    ...
    print("[1] Loading TSV")
    df = pd.read_csv('/mnt/data/vanichkinao/MetaPolisher/new_shufled_final_assembly.variants.tsv', sep="\t")
    df = df.fillna(0)
    ...
    param_grid = [
        dict(random_seed=11, depth=4),
        dict(random_seed=12, depth=4),
        dict(random_seed=13, depth=4),
        dict(random_seed=14, depth=4),
        dict(random_seed=25, depth=6),
        dict(random_seed=54, depth=6),
        dict(random_seed=453, depth=6),
        dict(random_seed=452, depth=8),
        dict(random_seed=457, depth=8),
        dict(random_seed=57, depth=8),
    ]
    ...
    for i, params in enumerate(param_grid):
        ...
        model = CatBoostClassifier(
            ...
            learning_rate=0.03,
            eval_metric="PRAUC", 
            loss_function="Focal:focal_alpha=0.5;focal_gamma=2",
            verbose=100,
            posterior_sampling=True,
            **params
        )
    ...
    df["ML_PROB_one"] = top_models[1]["model"].predict_proba(df[features])[:, 1]
    df["ML_PRED_one"] = (df["ML_PROB_one"] >= top_models[1]["threshold"]).astype(int)

    print("Финальный отчёт для одной модели\n\n", classification_report(df["TRUE_VARIANT"], df["ML_PRED_one"]))
    ...
    write_vcf(df_snv_sub, args.vcfs, f"{args.prefix}.snv_2.vcf", best_threshold, "ML_PROB")

    write_vcf(df_snv_sub, args.vcfs, f"{args.prefix}.one_model_2.vcf", top_models[1]["threshold"], "ML_PROB_one")
    ...
    df_shuffled = df.sample(frac=1, random_state=45).reset_index(drop=True)
    ...
    stack_features = [c for c in df.columns if c not in ["TRUE_VARIANT", "CHROM", "POS", "END", "REF", "ALT", "SVTYPE", "SVTYPE_NORM"]]

    X_train = train_df[stack_features]
    y_train = train_df["TRUE_VARIANT"]
    X_val   = val_df[stack_features]
    y_val   = val_df["TRUE_VARIANT"]
    ...
    for i, params in enumerate(param_grid):
        ...
        model2.fit(X_train, y_train, eval_set=(X_val, y_val), cat_features=cat_cols, use_best_model=True)

        # вероятности на валидации
        val_prob = model2.predict_proba(X_val)[:, 1]
    ...
    save_hyperparam_results(results2, f"{args.prefix}_hyperparams_2.tsv")

    N = 10
    top_models2 = sorted(results2, key=lambda x: x["recall"], reverse=True)[:N]

    val_probs_list = [m["model"].predict_proba(X_val)[:, 1] for m in top_models2]
    val_probs_ens = np.mean(val_probs_list, axis=0)
    ...
    # извлекаем модели и их recall
    models2 = [x["model"] for x in top_models2]
    recalls2 = np.array([x["recall"] for x in top_models2])

    # нормируем веса
    weights2 = recalls2 / recalls2.sum()

    print("Weights:", weights2)


    test_prob_one = top_models2[1]["model"].predict_proba(test_df[features])[:, 1]

    test_pred_one = (test_prob_one >= top_models2[1]["threshold"]).astype(int)
    ...
    # взвешенное усреднение
    test_prob = np.average(test_probs_list, axis=0, weights=weights2)

    # бинарные предсказания
    test_pred = (test_prob >= best_threshold2).astype(int)
    ...
    all_probs_list = [
        m["model"].predict_proba(df[features])[:, 1]
        for m in top_models2
    ]

    probs = np.vstack(all_probs_list)          # (n_models, n_variants)
    w = np.array(weights2, dtype=float)        # (n_models,)

    df["ML_PROB_final"] = np.average(probs, axis=0, weights=w)
    df["ML_PRED_final"] = (df["ML_PROB_final"] >= best_threshold2).astype(int)

    print("Финальный отчёт для финального ансамбля\n\n", classification_report(df["TRUE_VARIANT"], df["ML_PRED_final"]))

    df["ML_PROB_one_final"] = top_models2[1]["model"].predict_proba(df[features])[:, 1]
    df["ML_PRED_one_final"] = (df["ML_PROB_one_final"] >= top_models2[1]["threshold"]).astype(int)

    print("Финальный отчёт для одной модели\n\n", classification_report(df["TRUE_VARIANT"], df["ML_PRED_one_final"]))
    ...
    write_vcf(df_snv_sub, args.vcfs, f"{args.prefix}.snv.vcf", best_threshold, "ML_PROB_final")

    write_vcf(df_snv_sub, args.vcfs, f"{args.prefix}.one_model.vcf", top_models2[1]["threshold"], "ML_PROB_one_final")
